...
// config/scripts/ and reached through this one registry, instead of being hand-built as a C++ string
// literal at each call site. Two problems that used to recur are solved centrally here:
//
...
// `render()` appends the params object, so the script receives its arguments as one object `p` and
// the call site never builds JS text. A no-arg script still takes `p` (it ignores it) and is invoked
// with `{}`. Keeping the file a valid standalone expression means it lints and can be exercised on
// its own.

// A JSON object literal builder for a script's `p` argument. Numbers/bools/strings/arrays are encoded
// once, here, so escaping is not re-invented per feature.
class ScriptParams {
 public:
  ...
  // rows (e.g. the controls card's control/action pairs) that must not be pre-escaped by the caller.
...
// Process-wide registry of the page scripts. One instance (a singleton), initialised to the embedded
// defaults on first use and optionally pointed at a runtime override dir in main().
class ScriptLibrary {
 public:
