...
struct Config {
  ...
  // Fixed-interval skip (findings input-ux §18). A trigger bound to `skip_back`/`skip_fwd` jumps
  // the player by this many seconds via the player's own seekBy() over CDP — chapter-aware seeking
  // is unbuildable on TVHTML5 (m138.md S0.6), so this is the plain ±N s console model.
  // Hot-swappable.
  int skip_seconds = 10;
  ...
  // Self-update via the Flatpak portal (findings durable/self-update.md). OFF by default: the
  // portal path is not yet verified on a Deck, and self-updating is something a user should opt
  // into. When on, the launcher asks the portal to update ONLY this app, from its own `deckback`
  // remote, in the background (no root, no password); the new version applies on the NEXT launch.
